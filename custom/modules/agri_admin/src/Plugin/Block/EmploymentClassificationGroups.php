...
//		$config          = $this->getConfiguration();
    $config = $this->getConfiguration(); 
    $block['#id'] = $config['id']; 
    $block['#title'] = 'Employment Classification Groups Display'; 
    ...
    if (!$emplNode){
		...
		if ($paragraph_field_items = $emplNode->get('field_classification')->getValue()) {
		  ...
		  foreach ($paragraphs_objects as $paragraph) {
			...
			else{   $tmpText = $groupval.'-'.$levelval;	                }
			
			if ( strlen($classification) >0 ) {
			       	if ($currentlangId == 'en') {
                                    $classification = $classification.' or '. $tmpText;      }
                                else {   $classification = $classification.' ou '. $tmpText; }
                          } else {   $classification = $tmpText;	                     }     
		  }
	    ...
	    $classification = '<div class="field--label">Classification</div>'.'<div class="field--items">'.$classification.'</div>';
